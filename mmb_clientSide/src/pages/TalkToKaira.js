...
const TalkToKaira = () => {
  ...
  const [messagest1,setMessagest1]=useState([{"role": "system", "content": "Be sympathetic to the user. write only 5-6 words"}]);
  const [messagest2,setMessagest2]=useState([{"role": "system", "content": "Give answers in 10-15 words. Gently Ask counter questions over what the user has told. Do not show sympathy."}]);
  useLayoutEffect(() => {
    window.scrollTo(0, 0)
  })
  ...
  return (
    <div>
      ...
      <div className="transcript">{transcript}</div>
        ...
        <div className='openup-express' onClick={ handleChildVariable2}>Next</div>
      </div>
      <Footer></Footer>
    </div>
  )
